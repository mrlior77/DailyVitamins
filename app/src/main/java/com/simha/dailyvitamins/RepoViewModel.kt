...
class MainViewModel(
    ...
    private val KEY_WORKOUT = booleanPreferencesKey("workout_" + todayKey())
    ...
    private suspend fun refresh() {
        ...
        val dow = LocalDate.now(tz).dayOfWeek.name.take(3).uppercase() // MON TUE WED...
        ...
        for (a in assigns) {
            ...
            map.getOrPut(a.dayPart) { mutableListOf() }
                .add(TodayEntry(item, a.dayPart, sort))
        }
    ...
    private suspend fun ensureSeed() {
        ...
        // Seed ~17 items with default order; user can edit in future settings screen (to be expanded).
        val names = listOf(
        ...
        // Assign basic: split across day parts; duplicates on M/W/F for one vitamin (e.g., "ויטמין C" twice)
        ...
        // Example distribution (user can edit later in Settings screen to be added):
        add("מולטי-ויטמין", DayPart.WAKE)
        add("ויטמין D", DayPart.MORNING)
        add("ויטמין C", DayPart.MORNING)
        add("ויטמין C", DayPart.NIGHT, slotIndex = 1, daysJson = "[\"MON\",\"WED\",\"FRI\"]") // twice on M/W/F
