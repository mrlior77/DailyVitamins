...
class HomeWidgetProvider : AppWidgetProvider() {
    ...
    private fun updateWidget(context: Context, mgr: AppWidgetManager, id: Int) {
        ...
        // Simple remaining text (for brevity, not re-deriving full list)
    ...
    companion object {
        fun refreshAll(context: Context) {
            ...
            val cn = ComponentName(context, HomeWidgetProvider::class.java)
            val ids = mgr.getAppWidgetIds(cn)
            for (id in ids) updateAll(context, mgr, id)
        }
        private fun updateAll(ctx: Context, mgr: AppWidgetManager, id: Int) {
            val views = RemoteViews(ctx.packageName, R.layout.widget_home)
            mgr.updateAppWidget(id, views)
        }
    }
}
